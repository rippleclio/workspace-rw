...
(cd "$ROOT_DIR/wabifair-storefront-web" && npm run dev) &

echo "[2/4] wabifair-admin-console   (port 3001)"
(cd "$ROOT_DIR/wabifair-admin-console" && npm run dev) &

echo "[3/4] rippleclio-web           (port 5173)"
(cd "$ROOT_DIR/rippleclio-web" && npm run dev) &

echo "[4/4] rippleclio-admin-console (port 5174)"
(cd "$ROOT_DIR/rippleclio-admin-console" && npm run dev) &
...
echo
echo "Use bash scripts/stop-frontends.sh to shut them all down."
